#!/usr/bin/env python3
"""Runs a training shell script over a gamma sweep, each value repeated
across multiple seeds, and summarizes the results.

1-D sibling of run_train_grid_seed.py (same resumable/multi-seed idea,
see that file's docstring), for the peak-preserving SETS variant, which
is driven by a single --gamma instead of --alpha/--beta. Cells with an
already-complete log are skipped by default (pass --force to redo
everything) -- see run_train_grid_seed.py for why.

Usage:
    python run_train_grid_seed_peak_preserving.py \\
        run_train_dynamic_grid_peak_preserving.sh \\
        --gammas 0.25 0.5 0.75 1.0 \\
        --seeds 0 1 2 \\
        --profile libri_light_1hr --log-dir grid_logs_1hr_peak_preserving
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "script",
        help="Training shell script accepting `bash <script> <gamma> "
             "<seed> <profile>` (see "
             "run_train_dynamic_grid_peak_preserving.sh).")
    parser.add_argument(
        "--gammas", type=float, nargs="+", required=True,
        help="Gamma values to sweep.")
    parser.add_argument(
        "--seeds", type=int, nargs="+", default=[0, 1, 2],
        help="Seeds per gamma value (default: 0 1 2).")
    parser.add_argument(
    ...
    parser.add_argument(
        "--log-dir", type=str, default="grid_logs_peak_preserving",
...
def run_one(script: str, gamma: float, seed: int, profile: str,
           log_path: Path, tag: str) -> Dict[str, Any]:
    """Runs `bash <script> <gamma> <seed> <profile>`; see
    ...
    proc = subprocess.Popen(
        ["bash", script, str(gamma), str(seed), profile],
...
def print_sweep(profile: str, gammas: List[float],
                cell_summaries: Dict[float, Dict[str, Any]]) -> None:
    print(f"\n{'='*70}\neval_wer vs gamma, {profile} "
          f"(mean ± std over seeds)\n{'='*70}")
    for g in gammas:
        s = cell_summaries.get(g, {})
        if "eval_wer" in s:
            m, sd = s["eval_wer"]["mean"], s["eval_wer"]["std"]
            print(f"  gamma={g:<6.3f} eval_wer={m:.4f} ± {sd:.4f}")
        else:
            print(f"  gamma={g:<6.3f} n/a")
...
def main() -> None:
    ...
    total_runs = len(args.gammas) * len(args.seeds)
    print(f"Sweep ({args.profile}): {len(args.gammas)} gammas x "
          f"{len(args.seeds)} seeds = {total_runs} runs total")

    # Load any existing summary.json (e.g. from a prior sweep over a
    # different set of --gammas against this same --log-dir) so this run
    ...
    if summary_path.exists():
        ...
        for key, s in prev.get("summary", {}).items():
            g = float(key.rsplit("gamma=", 1)[1])
            cell_summaries[g] = s
    run_idx = 0
    n_skipped = 0

    for gamma in args.gammas:
        cell_key = f"profile={args.profile}_gamma={gamma}"
        cell_results = []
        for seed in args.seeds:
            run_idx += 1
            log_path = (log_dir /
                       f"{args.profile}_gamma{_fmt(gamma)}_seed{seed}.log")
            ...
            if cached is not None:
                ...
                result = run_one(args.script, gamma, seed, args.profile,
                                 log_path, tag)
        ...
        cell_summaries[gamma] = cell_summary
        ...
        with open(summary_path, "w") as f:
            json.dump({
                ...
                "results": all_results,
                "summary": {
                    f"profile={args.profile}_gamma={g}": s
                    for g, s in cell_summaries.items()
                },
            }, f, indent=2)

    print(f"\n{n_skipped}/{total_runs} runs were skipped (already-completed "
          f"cells reused).")
    print_sweep(args.profile, args.gammas, cell_summaries)
