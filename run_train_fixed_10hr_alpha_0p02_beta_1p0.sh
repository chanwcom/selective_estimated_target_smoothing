...
# calls this as `bash run_train_fixed.sh <seed>` for multi-seed comparisons;
# --seed also feeds the auto-generated --run_name, so repeats with
# different seeds land in separate checkpoint dirs instead of overwriting
# each other.
SEED=${1:-42}
python wav2vec2_finetuning_sets.py \
--alpha=0.02  \
