...
# calls this as `bash run_train_fixed_100hr_alpha_0p02_beta_1p0.sh <seed>`
# for multi-seed comparisons; --seed also feeds the auto-generated
# --run_name, so repeats with different seeds land in separate checkpoint
# dirs instead of overwriting each other.
SEED=${1:-42}
python wav2vec2_finetuning_sets.py \
--alpha=0.02  \
--beta=1.0   \
